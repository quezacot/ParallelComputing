...
def irec(cart, source, update):
    if source != MPI.PROC_NULL:
        rbuf = np.empty([1,Nxpart*Nypart])
        request = cart.Irecv(buf=rbuf, source=source)
        status = MPI.Status()
        request.Wait(status)
        read = pickle.loads(rbuf)
        return read
    else:
        return update
...
def set_ghost_points(u, cart):
    ...
    # The parallel part
    #up
    cart.isend(obj=u[1,:], dest=up)
    u[-1,:]= irec(cart, down, u[-1,:])
    #down
    cart.isend(obj=u[-2,:], dest=down)
    u[0,:]= irec(cart, up, u[0,:])
    #left
    cart.isend(obj=u[:,1], dest=left)
    u[:,-1] = irec(cart, right, u[:,-1])
    #right
    cart.isend(obj=u[:,-2], dest=right)
    u[:,0] = irec(cart, left, u[:,0])
